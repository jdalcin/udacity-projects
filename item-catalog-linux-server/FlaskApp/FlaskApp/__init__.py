...
from setup_database import Base, Users, Item
...
def get_user_id(name = None, email = None):
	if not name and not email:
		...
		email = session.query(Users.email).filter(func.lower(Users.name) == name.lower()).scalar()
	if email:
		user_id = session.query(Users.id).filter(func.lower(Users.email) == email.lower()).scalar()
		return user_id
	else:
		return None
...
def update_user_session_credentials(user_id):
	...
	login_session['picture'] = session.query(Users.picture).filter(Users.id == user_id).scalar()
	login_session['username'] = session.query(Users.name).filter(Users.id == user_id).scalar()
...
def upload_file(file, file_id, subfolder):
	if file and allowed_file(file.filename):
		...
		return os.path.join('/static/images/uploads', subfolder, filename)
	else:
		return None
...
def registration_screen():
	if request.method == 'GET':
		...
		if not session.query(exists().where(func.lower(Users.name) == name_lowercase)).scalar():
			if not registration_details.get('email') or not session.query(exists().where(func.lower(Users.email) == email_lowercase)).scalar():
...
def item_page(item_id):
	if request.method == 'GET':
		...
		if not item:
			...
			response.headers['Content-Type'] = 'application/json'
			return response
		owner = session.query(Users).filter(Users.id == item.user_id).scalar()
...
def latest_items_JSON():
	if request.method == 'GET':
		...
		for item in latest_items:
			item_owner = session.query(Users.name).filter(Users.id == item.user_id).scalar()
